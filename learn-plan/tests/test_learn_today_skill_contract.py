...
class LearnTodaySkillContractTest(unittest.TestCase):
    def test_skill_contract_describes_case_courseware_markdown(self) -> None:
        ...
        self.assertIn("生成练习题（四步流程）", text)
        self.assertIn("范围规划（子 Agent A）", text)
        self.assertIn("出题规划（子 Agent B）", text)
        self.assertIn("生成题目（子 Agent C）", text)
        self.assertIn("审题（子 Agent D）", text)
